				...
				Ext.util.Observable,
				{
					...
					onBeforeLoad : function() {
						if (!this.disabled) {
							this.el
									.mask(
											'<div class="x-loading-spinner"><span class="x-loading-top"></span><span class="x-loading-right"></span><span class="x-loading-bottom"></span><span class="x-loading-left"></span></div><div class="x-loading-msg">'
													+ this.msg + "</div>",
											this.msgCls, false);
		...
		.extend(
				Ext.data.Reader,
				{
					...
					extractData : function(a, c) {
						...
						return Ext.data.XmlReader.superclass.extractData.call(
								this, e, c)
					},
				...
				Ext.util.Observable,
				{
					...
					getOrientation : function() {
						...
						if (window.hasOwnProperty("orientation")) {
							return (window.orientation == 0 || window.orientation == 180) ? "portrait"
									: "landscape"
						} else {
							if (!Ext.is.iOS && !Ext.is.Desktop) {
								if ((a.width == this.lastSize.width && a.height < this.lastSize.height)
...
Ext.util.Draggable = Ext
		.extend(
				Ext.util.Observable,
				{
					...
					setStyleOffset : function(a) {
						this.getProxyEl().setLeft(a.x);
						this.getProxyEl().setTop(a.y);
						return this
					},
					startAnimation : function(c, a) {
						this.stopAnimation();
						var b = Date.now();
						...
						this.animationTimer = Ext.defer(
								this.handleAnimationFrame, 0, this);
						return this
					},
					...
					handleAnimationFrame : function() {
						...
						this.setOffset(a);
						this.animationTimer = Ext.defer(
								this.handleAnimationFrame, 10, this);
					...
					updateBoundary : function(f) {
						...
						if (this.useCssTransform) {
							...
							g = new Ext.util.Offset(
									this.getProxyEl().getLeft(), this
											.getProxyEl().getTop())
						}
							...
							Ext
									.iterate(
											...
											function(h, g) {
												...
												g
														.setOffset(this.indicatorOffsets[h]
	...
	Ext.util.Scroller = Ext
			.extend(
					Ext.util.Draggable,
					{
						...
						startMomentumAnimation : function(h) {
							var g = (h.event.originalTimeStamp) ? h.event.originalTimeStamp
									: h.time, f = g - this.originalStartTime;
							if ((!this.momentum || !(f <= this.startMomentumResetTime))
									&& !this.offsetBoundary
											.isOutOfBound(this.offset)) {
								return false
							}
							var d = this.minVelocityForAnimation, b, c = this.offset
									.copy(), a;
							this.isBouncing = {
								x : false,
								y : false
							...
							this.momentumAnimationStartTime = h.time;
							...
							this.momentumAnimationStartOffset = c;
							[ "x", "y" ]
									.forEach(
											function(e) {
														...
														this.bouncingAnimation[e]
																.set({
																	startTime : this.momentumAnimationStartTime
																			- ((1 / this.springTension) * this.acceleration),
																	startOffset : a,
																	startVelocity : b
																});
							...
							if (this.isDecelerating.x || this.isDecelerating.y
								...
								this.momentumAnimationTimer = Ext.defer(
										this.handleMomentumAnimationFrame, this
												.getFrameDuration(), this);
								return true
							}
							return false
						},
						...
						handleMomentumAnimationFrame : function() {
							...
							this.momentumAnimationTimer = Ext.defer(
									this.handleMomentumAnimationFrame, this
											.getFrameDuration(), this);
							var c = Date.now(), e = this.offset.copy(), b, a, d;
							[ "x", "y" ].forEach(function(f) {
								if (this.isDecelerating[f]) {
									e[f] = this.decelerationAnimation[f]
											.getOffset();
									b = this.momentumAnimationStartVelocity[f]
											* this.decelerationAnimation[f]
													.getFrictionFactor();
									d = this.offsetBoundary
											.getOutOfBoundOffset(f, e[f]);
									if (d != 0) {
										a = this.offsetBoundary.restrict(f,
												e[f]);
										if (this.bounces && this.bounces[f]) {
											this.bouncingAnimation[f].set({
												startTime : c,
												startOffset : a,
												startVelocity : b
											});
											this.isBouncing[f] = true;
									...
									if (this.isBouncing[f]) {
										e[f] = this.bouncingAnimation[f]
												.getOffset();
										a = this.offsetBoundary.restrict(f,
												e[f]);
										if (Math.abs(e[f] - a) <= 1) {
											this.isBouncing[f] = false;
...
Ext.gesture.Manager = new Ext.AbstractManager(
		{
			...
			attachListeners : function() {
				Ext.iterate(this.eventNames, function(b, a) {
					document.body.addEventListener(a, this.listenerWrappers[b],
							false)
				}, this);
				if (Ext.supports.Touch) {
			...
			onTouchStart : function(d) {
				var a = [], c = d.target, b = this;
				if (d.stopped === true) {
					return
				}
				if (!Ext.is.iOS) {
					if (!(c.tagName && [ "input", "textarea", "select" ]
							.indexOf(c.tagName.toLowerCase()) !== -1)) {
						d.preventDefault()
					}
				}
				if (this.isFrozen) {
				...
				this.locks = {};
				this.currentTargets = [ c ];
				while (c) {
					if (this.targets.indexOf(c) !== -1) {
						a.unshift(c)
					}
					c = c.parentNode;
					this.currentTargets.push(c)
				}
				this.startEvent = d;
				this.startPoint = Ext.util.Point.fromEvent(d);
				this.lastMovePoint = null;
				this.isClick = true;
				this.handleTargets(a, d)
			},
...
Ext.gesture.Gesture = Ext.extend(Object, {
	...
	fire : function(d, g, a) {
		var c = this.listeners[d], f = c && c.length, b;
...
Ext.gesture.Drag = Ext
		.extend(
				Ext.gesture.Touch,
				{
					...
					vertical : false,
					constructor : function() {
						Ext.gesture.Drag.superclass.constructor.apply(this,
								arguments);
						if (this.direction == "both") {
							this.horizontal = true;
							this.vertical = true
					...
					onTouchMove : function(b, c) {
						...
						if (!this.dragging) {
							if (this.isDragging(a)
									&& this.fire("dragstart", b, a)) {
								this.dragging = true;
				...
				Ext.gesture.Gesture,
				{
					...
					onTouchStart : function(f) {
						...
						if (Ext.supports.Touch && f.targetTouches.length >= 2) {
							b.lock("swipe", "scroll", "scrollstart",
									"scrollend", "touchmove", "touchend",
									"touchstart", "tap", "tapstart", "taphold",
									"tapcancel", "doubletap");
							b.pinching = true;
							var c = f.targetTouches, d = b.firstTouch = c[0], a = b.secondTouch = c[1];
							b.previousDistance = b.startDistance = b
									.getDistance();
							b.previousScale = 1;
							b.fire("pinchstart", f, {
							...
							if (b.pinching) {
								b.unlock("swipe", "scroll", "scrollstart",
										"scrollend", "touchmove", "touchend",
										"touchstart", "tap", "tapstart",
										"taphold", "tapcancel", "doubletap");
								b.pinching = false
							}
						}
					},
					...
					getPinchInfo : function() {
						var b = this, f = b.getDistance(), e = f
								/ b.startDistance, d = b.firstTouch, a = b.secondTouch, c = {
							scale : e,
							deltaScale : e - 1,
					...
					getDistance : function() {
						var a = this;
						return Math.sqrt(Math.pow(Math.abs(a.firstTouch.pageX
								- a.secondTouch.pageX), 2)
								+ Math.pow(Math.abs(a.firstTouch.pageY
										- a.secondTouch.pageY), 2))
					}
				});
		...
		.extend(
				Ext.lib.Component,
				{
					...
					setScrollable : function(a) {
						...
						this.scroller = null;
						if (a !== false) {
							var b = Ext.isObject(a) ? a.direction : a;
							a = Ext.apply({}, Ext.isObject(a) ? a : {}, {
								direction : b
							});
							if (!this.scrollEl) {
								this.scrollEl = this.getTargetEl()
										.createChild();
							...
							this.scroller = (new Ext.util.ScrollView(
									this.scrollEl, a)).scroller
						} else {
...
Ext.Container = Ext
		.extend(
				Ext.lib.Container,
				{
					...
					onFieldFocus : function(g, f) {
						var d = this.getClosestScroller(), h = Ext.util.Region
								.from(d.containerBox), k = g.fieldEl
								.getPageBox(true);
						if (this.focusingField == g || !Ext.is.iOS) {
							if (Ext.is.iOS && window.pageYOffset == 0) {
								window.scrollTo(0, 0)
							}
							var i = new Ext.util.Offset();
							if (k.left < h.left) {
								i.x = h.left - k.left
							}
							if (k.top < h.top) {
							...
							if (!i.isZero()) {
								var j = this.getLastWindowScroll();
								d.scrollBy(i);
								if (Ext.is.iOS) {
							...
							if (this.lastFocusedField) {
								var a = k.top
										- this.lastFocusedField.fieldEl.getY(), b = d.offset.y
										- a, c = false;
								if (!h.contains(k)
										&& (b != 0 || (b == 0 && d.offset.y != 0))) {
									c = true
								}
								if (b > 0) {
									b = 0
		...
		.extend(
				Ext.DataView,
				{
					...
					initComponent : function() {
						...
						this.tpl = '<tpl for="."><div class="x-list-item"><div class="x-list-item-body">'
								+ this.itemTpl + "</div>";
		...
		.extend(
				Ext.Component,
				{
					...
					onMaskTap : function(a) {
						...
						if (Ext.is.iOS && a.browserEvent
								&& !a.browserEvent.isSimulated
								&& !a.browserEvent.isSimulated) {
...
Ext.form.Slider = Ext.extend(Ext.form.Field, {
	...
	onOrientationChange : function() {
		...
		if (a.dragObj) {
			a.dragObj.updateBoundary();
			this.moveThumb(a, this.getPixelValue(a.getValue(), a), 0)
		}
	},
...
Ext.form.Select = Ext.extend(Ext.form.Text, {
	...
	showComponent : function() {
		if (Ext.is.Phone) {
			...
			var b = this.getListPanel(), a = this.store.findExact(
					this.valueField, this.value);
			b.showBy(this.el, "fade", false);
			b.down("#list").getSelectionModel().select(a != -1 ? a : 0, false,
					true)
		}
		this.isActive = true
	},
